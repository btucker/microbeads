...
def _generate_semantic_summary(issue: dict[str, Any]) -> str | None:
    """Generate a semantic summary of an issue using Claude Haiku.

    Uses the same structured format as beads: Summary, Key Decisions, Resolution.
    ...
    try:
        ...
        # Build issue context for summarization (matching beads format)
        prompt_parts = [
        ...
        if issue.get("design"):
            prompt_parts.extend(["", "**Design:**", issue["design"]])

        if issue.get("acceptance_criteria"):
            prompt_parts.extend(["", "**Acceptance Criteria:**", issue["acceptance_criteria"]])

        if issue.get("notes"):
            prompt_parts.extend(["", "**Notes:**", issue["notes"]])
